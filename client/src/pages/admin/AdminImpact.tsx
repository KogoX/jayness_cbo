...
import type { GalleryItem, Testimonial } from '../../types/impact.types';
...
const AdminImpact: React.FC = () => {
  ...
  const fetchData = async () => {
    try {
      const [tRes, gRes] = await Promise.all([apiClient.get('/impact/testimonials'), apiClient.get('/impact/gallery')]);
            ...
            <form onSubmit={handleAddTestimonial} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <input
                  ...
                  value={tForm.role}
                  onChange={(e) => setTForm({ ...tForm, role: e.target.value })}
                  required
                />
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {testimonials.map((t) => (
              <div key={t._id} className="bg-white p-4 rounded shadow flex items-start gap-4 border">
                ...
                <button onClick={() => handleDelete('testimonials', t._id)} className="text-red-500 hover:text-red-700" aria-label="Delete testimonial">
        ...
        <div className="space-y-8">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-100">
            ...
            <form onSubmit={handleAddGallery} className="space-y-4">
              ...
              <input
                placeholder={gForm.type === 'video' ? 'YouTube Embed URL' : 'Image URL'}
                className="w-full border p-2 rounded"
                value={gForm.src}
                onChange={(e) => setGForm({ ...gForm, src: e.target.value })}
                required
              />
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {gallery.map((g) => (
              <div key={g._id} className="relative group bg-white rounded shadow overflow-hidden">
                {g.type === 'video' ? (
                  <div className="h-32 bg-black flex items-center justify-center text-white gap-2 text-sm">
                ...
                <div className="p-2 text-xs flex justify-between items-center">
                  ...
                  <button onClick={() => handleDelete('gallery', g._id)} className="text-red-500 font-bold">
                    Delete
                  </button>
                </div>
              </div>
